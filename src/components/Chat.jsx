            ...
            <div className={`flex w-full h-20 ${props.selectedID === props.id ? 'bg-[#5B96F7]' : 'bg-white'} rounded-md shrink-0 drop-shadow-lg  p-4`}>
                <div className='flex w-full items-center gap-2'>
                    ...
                    <div className='w-1/2 flex flex-col gap-1 items-start overflow-hidden'>
                        {/* Name */}
                        <h1 className={`${props.selectedID === props.id ? 'text-white' : 'text-black'} font-semibold text-sm text-start`}>{props.name}</h1>
                        {/* Message */}
                        <span className={`${props.selectedID === props.id ? 'text-white' : 'text-black'} text-xs w-full font-medium overflow-hidden whitespace-nowrap text-start`}>{props.msg}</span>
                    </div>
                    ...
                    <div className='w-1/4 flex flex-col gap-1 items-center'>
                        {/* Time */}
                        <span className={`${props.selectedID === props.id ? 'text-white' : 'text-black'} text-sm`}>{props.time}</span>
